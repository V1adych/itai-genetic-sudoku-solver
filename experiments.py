...
def main():
    parser = argparse.ArgumentParser(description="Solve Sudoku using Genetic Algorithm")
    parser.add_argument(
        "--input-dir", required=True, help="Input dir containing the Sudoku puzzles"
    )
    parser.add_argument(
        "--output-path",
        required=True,
        help="Output file to write the fitness history in JSON format",
    )
    args = parser.parse_args()

    histories = {}

    for difficulty in Path(args.input_dir).iterdir():
        for input_file in difficulty.iterdir():
            ...
            histories[f"{difficulty.name}/{input_file.name}"] = history
